...
class EditInfoHospitalSelectViewController: UIViewController,QNInterceptorProtocol, UITableViewDataSource, UITableViewDelegate {
    ...
    func fetchHospitalItems() {
        QNNetworkTool.fetchHospitalItems("", province_id: self.province_id!, city_id: self.city_id!, area_id: "") { (
            array, error, string) -> Void in
            if array != nil {
               self.hospitalList = array!
                self.tableView.reloadData()
            } else {
                QNTool.showErrorPromptView(nil, error: error, errorMsg: nil)
            }
        }
    }
}
