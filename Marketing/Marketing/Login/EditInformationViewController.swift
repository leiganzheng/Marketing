...
//    @IBOutlet weak var textField1: UITextField!  //地区
    ...
    @IBAction func done(sender: UIButton!) {
        if self.check() {
            ...
            QNNetworkTool.saveDetailDoctor(g_doctor!.doctorId,province_id: self.province_id!,
                city_id: self.city_id!,
                area_id: "", hospital_id: self.hospital_id!, department_id: self.dep_id!, job_title: self.textField4.text!, illness_id: self.ill_id!, completion: { (succeed, error, errorMsg) -> Void in
                    QNTool.hiddenActivityView()
                    if succeed {
                        QNTool.showPromptView("信息提交成功，工作人员会尽快与您联系！") { () -> Void in
                            self.finished!()
                        }
                    }
                    else {
